...
public class JwtOperator {
    ...
    public Claims getClaimsFromToken(String token) {
        try {
            return Jwts.parser().setSigningKey(this.reactiveLightSecurityProperties.getJwt().getSecret().getBytes())
    ...
    private Date getExpirationTime() {
        return new Date(
                System.currentTimeMillis() + reactiveLightSecurityProperties.getJwt().getExpirationInSecond() * 1000);
    }
    ...
    public String generateToken(User user) {
        ...
        return Jwts.builder().setClaims(claims).setIssuedAt(createdTime).setExpiration(expirationTime).signWith(key)
                .compact();
    }
