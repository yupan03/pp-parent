...
public class ReactiveUserOperator {
    ...
    private User getUserFromToken(String token) {
        ...
        return User.builder().id((String) userId).username((String) username).roles((List<String>) roles).build();
    }
    ...
    private String getTokenFromRequest(ServerHttpRequest request) {
        List<String> headers = request.getHeaders().get(ConstantsSecurity.AUTHORIZATION_HEADER);
