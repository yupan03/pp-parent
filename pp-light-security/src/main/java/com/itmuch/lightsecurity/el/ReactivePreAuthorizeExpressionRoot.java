...
public class ReactivePreAuthorizeExpressionRoot {
    ...
    public Mono<Boolean> hasLogin() {
        return userOperator.getUser().map(user -> true).switchIfEmpty(Mono.just(false));
    }
    ...
    public Mono<Boolean> hasAllRoles(String... roles) {
        return userOperator.getUser().map(user -> {
    ...
    public Mono<Boolean> hasAnyRoles(String... roles) {
        return userOperator.getUser().map(user -> {
            ...
            boolean result = userRoles.stream().anyMatch(roleList::contains);
            if (!result) {
