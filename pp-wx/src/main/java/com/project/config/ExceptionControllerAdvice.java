...
public class ExceptionControllerAdvice{
	...
	@ResponseBody
	public ResultObj<?> handlerProjectException(CusException e) {
	...
	public ResultObj<?> handlerServletException(ServletException e) {
		if (e instanceof HttpRequestMethodNotSupportedException) {
			// 405请求方式错误
			return new ResultObj<>(StatusEnum.URL_METHOD_EOOR, e.getMessage());
		} else if (e instanceof NoHandlerFoundException) {
			// 404 请求未找到
			return new ResultObj<>(StatusEnum.URL_NOT_FOUND, e.getMessage());
		} else {
	...
	public ResultObj<?> handlerSQLException(SQLException e) {
		return new ResultObj<>(StatusEnum.ERROR_SQL, e.getMessage());
	}
}